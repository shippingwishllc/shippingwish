...
async function prerender() {
  ...
  console.log('[SSG] Pre-rendering LoadsNexus™ Static HTML5...');
  ...
  // Sync to public/loadsnexus/
  const publicDir = path.resolve(__dirname, '../../public/loadsnexus');
  if (fs.existsSync(publicDir)) {
    console.log(`[SSG] Synchronizing pre-rendered assets to ${publicDir}...`);
    
    // Copy index.html
    fs.writeFileSync(path.join(publicDir, 'index.html'), finalHtml, 'utf-8');

    // Copy dist/assets into public/loadsnexus/assets
